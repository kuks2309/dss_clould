...
**DSS 시뮬레이터 전용 LIO-SAM 포크** — [LIO-SAM](https://github.com/TixiaoShan/LIO-SAM)(Tixiao Shan, BSD-3) 의
livox_lio_sam ROS2 포크를 `dss_ros2_bridge` 스트림(`/dss/sensor/lidar3d`·`/dss/sensor/imu`) 위에서
돌도록 개조한 것. 알고리즘 4파일은 원본과 rename 외 동일하며, DSS 개조분은 다음과 같다:

| DSS 전용 | 내용 |
| --- | --- |
| `config/params_dss.yaml` | DSS 라이다 실측 기반 (16ch·500샘플·10 Hz) |
| `scripts/dss_imu_gravity.py` | DSS IMU 중력 합성 릴레이 (원천 IMU 가속도·각속도 0 우회) |
| `launch/slam/run_slam_dss.launch.py` | DSS 기동 (static TF·중력 릴레이 포함) |

```bash
ros2 launch dss_lio_sam run_slam_dss.launch.py            # DSS SLAM
ros2 launch dss_lio_sam run_slam_dss.launch.py rviz:=false use_imu_gravity:=false
```

설계 결정·리뷰·부채는 Divine 워크스페이스 `docs/`(adr·code_review/slam3d·debt) 참조.

---

# LIO-SAM (upstream README)
