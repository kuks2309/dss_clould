...
public:
  ...
  : Node("hdl_localization", options)
  {
    ...
    if (use_imu) {
      ...
      imu_sub = create_subscription<sensor_msgs::msg::Imu>("/gpsimu_driver/imu_data", rclcpp::SensorDataQoS().keep_last(256), std::bind(&HdlLocalizationNodelet::imu_callback, this, std::placeholders::_1));  // DSS BEST_EFFORT 정합
    }
    // DSS 브리지는 센서 스트림을 BEST_EFFORT(SensorDataQoS) 로 발행한다 — 기본 depth-5
    // (RELIABLE) 구독은 RxO 위반(offered < requested)으로 메시지가 한 건도 오지 않는다.
    points_sub = create_subscription<sensor_msgs::msg::PointCloud2>("/velodyne_points", rclcpp::SensorDataQoS().keep_last(5), std::bind(&HdlLocalizationNodelet::points_callback, this, std::placeholders::_1));
...
private:
  ...
  void globalmap_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    if(use_global_localization) {
      ...
      // 실행 중인 executor 의 콜백 컨텍스트다 — 여기서 spin_until_future_complete 를 부르면
      // "Node has already been added to an executor" abort(실측). 응답은 콜백으로 받는다.
      // (rclcpp::Node::SharedPtr(this) 도 소유권 없는 shared_ptr 라 이중 해제 폭탄 — 함께 제거)
      set_global_map_service->async_send_request(req,
        [this](rclcpp::Client<hdl_global_localization::srv::SetGlobalMap>::SharedFuture) {
          RCLCPP_INFO(get_logger(), "SetGlobalMap completed");
        });
    }
  }
  ...
  bool relocalize(std::shared_ptr<std_srvs::srv::Empty::Request> req, std::shared_ptr<std_srvs::srv::Empty::Response> res) {
    ...
    // 동일 사유(콜백 내 중첩 spin 금지)로 응답 콜백 체인으로 전환 — Empty 응답이므로
    // 핸들러는 즉시 반환하고, 재위치화 적용은 응답 도착 시 콜백에서 수행한다.
    query_global_localization_service->async_send_request(query_req,
      [this, scan](rclcpp::Client<hdl_global_localization::srv::QueryGlobalLocalization>::SharedFuture f) {
        this->apply_global_localization_result(f.get(), scan);
      });
    return true;
  }

  void apply_global_localization_result(
      std::shared_ptr<hdl_global_localization::srv::QueryGlobalLocalization::Response> query_result,
