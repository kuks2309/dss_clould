...
GlobalLocalizationBBS::GlobalLocalizationBBS(rclcpp::Node::SharedPtr node) : node(node) {
  // 포트가 원본의 bbs/ 프리픽스를 누락해 RViz 설정(bbs/gridmap)과 어긋났었다 — 정정
  gridmap_pub = this->node->create_publisher<nav_msgs::msg::OccupancyGrid>("bbs/gridmap", 1);
  map_slice_pub = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("bbs/map_slice", 1);
  scan_slice_pub = this->node->create_publisher<sensor_msgs::msg::PointCloud2>("bbs/scan_slice", 1);
  // 늦게 붙은 구독자 대응: 1 Hz 재발행
  gridmap_republish_timer = this->node->create_wall_timer(std::chrono::seconds(1), [this]() {
    if (last_gridmap) gridmap_pub->publish(*last_gridmap);
    if (has_map_slice) map_slice_pub->publish(last_map_slice);
  });
...
void GlobalLocalizationBBS::set_global_map(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  ...
  last_map_slice = msg; has_map_slice = true;          // 캐시 (타이머 재발행용)
  last_gridmap = bbs->gridmap()->to_rosmsg();
  map_slice_pub->publish(msg);
  gridmap_pub->publish(*last_gridmap);
}
