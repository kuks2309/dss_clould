...
public:
  GlobalLocalizationNode(const rclcpp::NodeOptions& options) : rclcpp::Node("global_localization_node", options) {
    // 포트 버그 수정: declare 반환값을 멤버에 대입하지 않아 해상도가 0 으로 남았고,
    // leaf-0 복셀이 지도 전체를 1점으로 붕괴시켰다(실측 진단 로그로 확인).
...
private:
  ...
  bool set_global_map(srv::SetGlobalMap::Request::SharedPtr req, srv::SetGlobalMap::Response::SharedPtr res) {
    ...
    RCLCPP_INFO_STREAM(get_logger(), "recv cloud: " << cloud->size() << " pts");  // 진단 로그
    cloud = downsample(cloud, globalmap_downsample_resolution);
    RCLCPP_INFO_STREAM(get_logger(), "after downsample(" << globalmap_downsample_resolution << "): " << cloud->size() << " pts");  // 진단 로그
