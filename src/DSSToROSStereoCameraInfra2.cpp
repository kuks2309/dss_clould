...
class DSSToROSStereoCameraInfra2Node : public rclcpp::Node {
public:
...
public:
    DSSToROSStereoCameraInfra2Node() : Node("DSSToROSStereoCameraInfra2Node") {
        ...
        timer_   = this->create_wall_timer(std::chrono::seconds(3), std::bind(&DSSToROSStereoCameraInfra2Node::onTick, this));
    ...
    ~DSSToROSStereoCameraInfra2Node() override
    {
    ...
    void publishHeartBeat()
    {
        ...
        natsStatus s = natsConnection_PublishString(nats_.conn, "dss.DSSToROSStereoCameraInfra2Node.heartBeat", message.dump().c_str());
        if (s != NATS_OK) {
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<DSSToROSStereoCameraInfra2Node>());
    rclcpp::shutdown();
}
