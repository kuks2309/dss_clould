...
class DSSToROSStereoCameraDepthNode : public rclcpp::Node {
public:
...
public:
    DSSToROSStereoCameraDepthNode() : Node("DSSToROSStereoCameraDepthNode") {
        ...
        timer_   = this->create_wall_timer(std::chrono::seconds(3), std::bind(&DSSToROSStereoCameraDepthNode::onTick, this));
    ...
    ~DSSToROSStereoCameraDepthNode() override
    {
    ...
    void publishHeartBeat()
    {
        ...
        natsStatus s = natsConnection_PublishString(nats_.conn, "dss.dssToROSStereoCameraDepth.heartBeat", message.dump().c_str());
        if (s != NATS_OK) {
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<DSSToROSStereoCameraDepthNode>());
    rclcpp::shutdown();
}
