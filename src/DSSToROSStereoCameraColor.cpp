...
class DSSToROSStereoCameraColorNode : public rclcpp::Node {
public:
...
public:
    DSSToROSStereoCameraColorNode() : Node("DSSToROSStereoCameraColorNode") {
        ...
        timer_   = this->create_wall_timer(std::chrono::seconds(3), std::bind(&DSSToROSStereoCameraColorNode::onTick, this));
        subscribeTopicRaw("dss.sensor.camera.stereo.color",
        ...
        pub_ = this->create_publisher<sensor_msgs::msg::Image>(
            "/dss/sensor/camera/color/image_raw", rclcpp::QoS(rclcpp::KeepLast(10)).reliable());
        RCLCPP_INFO(get_logger(), "[NATS]dss.sensor.camera.stereo.color → [ROS2]/dss/sensor/camera/color/image_raw");
    }

    ~DSSToROSStereoCameraColorNode() override
    {
    ...
    void publishHeartBeat()
    {
        ...
        natsStatus s = natsConnection_PublishString(nats_.conn, "dss.dssToROSStereoCameraColor.heartBeat", message.dump().c_str());
        if (s != NATS_OK) {
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<DSSToROSStereoCameraColorNode>());
    rclcpp::shutdown();
}
