...
class DSSToROSStereoCameraInfra1Node : public rclcpp::Node {
public:
...
public:
    DSSToROSStereoCameraInfra1Node() : Node("DSSToROSStereoCameraInfra1Node") {
        ...
        timer_   = this->create_wall_timer(std::chrono::seconds(3), std::bind(&DSSToROSStereoCameraInfra1Node::onTick, this));
        subscribeTopicRaw("dss.sensor.camera.stereo.infra1",
        ...
        // 비압축 rgb8 프레임은 조각화 전송을 타므로 best-effort 에서 샘플 단위 손실이 크다.
        // 이미지 토픽만 재전송이 있는 reliable 을 쓴다.
        pub_ = this->create_publisher<sensor_msgs::msg::Image>(
            "/dss/sensor/camera/infra1/image_raw", rclcpp::QoS(rclcpp::KeepLast(10)).reliable());
        RCLCPP_INFO(get_logger(), "[NATS]dss.sensor.camera.stereo.infra1 → [ROS2]/dss/sensor/camera/infra1/image_raw");
    }

    ~DSSToROSStereoCameraInfra1Node() override
    {
    ...
    void publishHeartBeat()
    {
        ...
        natsStatus s = natsConnection_PublishString(nats_.conn, "dss.DSSToROSStereoCameraInfra1Node.heartBeat", message.dump().c_str());
        if (s != NATS_OK) {
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<DSSToROSStereoCameraInfra1Node>());
    rclcpp::shutdown();
}
